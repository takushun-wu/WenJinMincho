...
## 2.001(2025.10.3)
#### 新增内容
- 添加1个汉字的假想G源字形：𲷇（`32DC7`）；
#### 改进与问题修复
- 将1个汉字修改为G标字形，与码表字形相一致（仅默认字形）：鿄；
- 将3个汉字修改为码表字形（仅默认字形）：㫚䝈曶；
- 修正12个汉字字形：𦯵𪪿𪽺𫃇𫉻𫓊𫙼𫚄𫚇𬶘𱤏（`cv02`类推国标字形）芝󠄁（`829D E0101`）；
- 优化7个汉字字形：𥆒𥋉𨰼𨰽𪾞𫅪𫖈；
- 修正31个汉字在加粗时存在部分笔画减细的现象：㮧䃖䡧嵨些梼龮龳龼鿅鿆鿊鿌鿐鿧鿪𠁨𠑥𡷊𤅄𤅕𤜥𦆮𨬫𪇟𬆮𬺭𲘿𲰹（默认及码表字形）𲾝。

## 2.000(2025.9.13)
### 🎉重要更新
#### 适配Unicode 17.0版本，以下内容发生变化：
- 新增4,316个汉字［**6**（扩展C）+**12**（扩展E）+**4,298（扩展J）**］，支持Unicode汉字总数已破十万；
- 跟随[G（中国大陆）源汉字横向扩展](https://www.unicode.org/irg/docs/n2729r3-ChinaHorizontalExtension.pdf)，超1,100个汉字字形修改成G源；
- 新增缩小字号的儿化音符号`U+16FF2,16FF3`“𖿲𖿳”（即缩小版的“儿”、“兒”）；
- 新增Unicode 17.0添加的国际音标上标字符`U+1AE0..1AE5`“◌᫠◌᫡◌᫢◌᫣◌᫤◌᫥”（即上标版的“◌̘◌̙◌̠◌̺◌̻◌̼”）；
- 新增沙特里亚尔的货币符号`U+20C1`“⃁”（采用Emran Alhaddad制作的Saudi Riyal Font字体的字形）；
- 其他少量汉字跟随Unicode升级而修改字形。
#### 其他较大的更新
- V（越南）源汉字样式大幅改进，修改约2,900汉字（不含扩展J）；
- 不存在G源且不支持假想G源的字形的字形标准顺序发生改变，KP（朝鲜）源位于H（中国香港）源前面，因此修改41个汉字字形（另有14个汉字仅修改了码表字形）；
- OpenType`cv02`标签的字形现遵循[GB/T 22321.1-2025《信息技术 中文编码字符集 汉字48点阵字型 第1部分：宋体》](https://openstd.samr.gov.cn/bzgk/gb/newGbInfo?hcno=421B9135604C56A2EA1C4396A941CFB4)国家标准（该标准没有的少数汉字类推），在此基础上新增了康熙部首（`U+2F00..2FD5`）的字形显示。

### 其他改变
#### 新增内容
- 新增7个旧版国际音标字符：ƈƙƥƭʞʠ◌̢
- 新增太极、八卦（包括1-3行卦象）、易经六十四卦、太玄经卦象符号的支持。
#### 改进与问题修复
- 基本区/扩展A区的全部汉字在默认情况下都为G标字形，修改了23个汉字：䶺䶼䶽䶿龦龨龩龬龮龳龼鿀鿁鿅鿆鿈鿊鿌鿐鿧鿪鿮鿯；
- 修改了5个汉字的字形：𦉪𤰉𬆮𭣧𱤏
- 修正55个汉字字形；
<details>
...
-   - **URO**：丗乆嵨慂樷橜†欎羀邳黴†齂
    ...
    - **古籍规范字（`ss12`）**：橜猒蕪郾顄鶠𣎗
    - **仅码表字形（`cv01`），默认字形不变**：甒顄饜黶
    - 画†的汉字Unicode码表字形（`cv01`）不变。
</details>
...
- 福禄寿喜囍财符号“🉠🉡🉢🉣🉤🉥”现取自[CJK Symbols](https://github.com/unicode-org/cjk-symbols)字体；
- 修复了在调用码表/GB字形（`cv01`/`cv02`）时部分中日韩兼容汉字出现字形连同被替换的问题；
- 分隔号“/／”现可参与标点压缩。

### 说明文件
- 使用手册重新设计排版样式，内容更加清晰可见；
- 使用手册添加第1章“快速安装指南”，让用户更加简单上手；
- 优化了部分HTML/CSS样例代码；
- 字形标准切换字形表现在以单独的PDF文件发布；
- 字形标准切换字形表与异体字序列表添加了英文版的说明。
## 1.100(2025.8.22)
### 新增内容
- 新增[CAAPH（文化艺术出版社）IVD异体字集](https://www.unicode.org/ivd/caaph/)的支持，共198个序列。
### 改进与问题修复
- 修正18个汉字字形；
<details>
<summary>点击以查看</summary>
...
-   - **URO**：添
    - **ExtB**：𡧗
    - **IVS**：添󠄀`6DFB E0100/E0102`、添󠄁`6DFB E0101/E0103`（与`6DFB`共用一个字形）
</details>

## 1.022(2025.7.4)
### 改进与问题修复
- 每个平面字体都添加字体名称相关字符，修复第2/3平面字体无法显示字体名称的问题。具体添加字符包含：体宋常平文明朝標津準第規规面體면명문반일조진평
- 优化以下非汉字字符：✓✗⿾⿿㇯
- 修正7个汉字字形；
<details>
<summary>点击以查看</summary>
...
-   - **ExtB**：𧍙
    - **ExtG**：𰲎𰸖𱋽
    - **ExtH**：𱝡
    - **IVS**：𣟱󠄀`237F1 E0100`、𭭋󠄀`2DB4B E0100`
</details>
...
- 修复第2/3平面字体在Word 2010中与宋体混排时出现向左偏移的现象

...
# WenJin Mincho (Preliminary Completion Test)
...
> [!WARNING]
> 
> This project is still in development (Preliminary Completion Test) and does not represent the final quality. Any questions or suggestions are welcome.
...
- Republish or install font files, embed them into any software or device, and the software or device can be redistributed or resold (if the installation package is created for the purpose of installing this font, it must be distributed free of charge, and NO PAYMENT can be made for profit, otherwise, it will be regarded as an act of selling/disguised selling of fonts).
- Any modifications, extensions and format conversions to the font files **must be published under the *SIL Open Font License***, and the **reserved name of the font (and its translated forms in other languages) must not be used: WenJin Mincho, <span lang="zh-Hans">文津宋体</span>, <span lang="zh-Hant">文津宋體</span>, <span lang="ja">文津明朝</span>, <span lang="ko">문진(文津) 명조</span>** (if only the source fonts are formatted, or subsetted, and the changed font files are not released separately, the reserved name may continue to be used).
### You cannot do...
